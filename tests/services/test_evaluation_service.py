...
class TestBuildEvaluationPrompt:
    ...
    def test_build_evaluation_prompt(self):
        ...
        assert previous_text in user_message
    ...
    def test_build_evaluation_prompt_section_order(self):
        ...
        処方_pos = user_message.index("<前回の記載>")
...
class TestValidateAndGetPrompt:
    ...
    @patch("app.services.evaluation_service.settings")
    def test_claude_evaluation_model_without_anthropic_model_returns_error(
        self, mock_settings, _mock_validate
    ):
        """EVALUATION_MODEL=Claude でANTHROPIC_MODEL未設定の場合はエラーを返す"""
        mock_settings.max_input_tokens = 100000
        ...
        assert prompt is None
        assert error == MESSAGES["CONFIG"]["CLAUDE_MODEL_NOT_SET"]
    ...
    @patch("app.services.evaluation_service.settings")
    def test_claude_evaluation_model_returns_prompt(
        self, mock_settings, mock_db_session, _mock_validate
    ):
        """EVALUATION_MODEL=Claude でANTHROPIC_MODEL設定済の場合はプロンプトを返す"""
        mock_settings.max_input_tokens = 100000
        mock_settings.evaluation_model = "Claude"
    ...
    @patch("app.services.evaluation_service.settings")
    def test_prompt_injection_in_output_returns_error(
        self, mock_settings, _mock_validate
    ):
    ...
    @patch("app.services.evaluation_service.get_db_session")
    @patch("app.services.evaluation_service.settings")
    def test_no_prompt_in_db_returns_error(
        self, mock_settings, mock_db_session, _mock_validate
    ):
    ...
    @patch("app.services.evaluation_service.get_db_session")
    @patch("app.services.evaluation_service.settings")
    def test_success_returns_prompt_content(
        self, mock_settings, mock_db_session, _mock_validate
    ):
